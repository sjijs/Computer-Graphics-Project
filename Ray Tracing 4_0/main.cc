...
void final_scene(int image_width, int samples_per_pixel, int max_depth) {
    ...
    auto light = make_shared<diffuse_light>(color(7, 7, 7));
    auto area_light = make_shared<quad>(point3(123,554,147), vec3(300,0,0), vec3(0,0,265), light);
...
void test_annulus() {
    ...
    auto key_light = make_shared<sphere>(point3(-2, 8, -3), 1.5, light_material);
    world.add(key_light);
...
void test_model() {
    ...
    auto key_light = make_shared<sphere>(point3(0, 5, 4), 1.2, light);
    world.add(key_light);
    lights->add(key_light);
...
// 4.0 新功能综合测试场景：
// 1) Disney 基础材质参数（metallic/roughness/clearcoat/sheen）
// 2) NEE + MIS（通过显式 light_sampler）
// 3) 外部模型导入 + 贴图覆盖材质渲染
void feature_showcase_4_0() {
    hittable_list world;
    auto lights = make_shared<hittable_list>();

    // 地面
    auto ground_tex = make_shared<checker_texture>(0.6, color(0.18, 0.18, 0.20), color(0.80, 0.80, 0.82));
    world.add(make_shared<sphere>(point3(0, -1000, 0), 1000, make_shared<lambertian>(ground_tex)));

    // 主光源（面光源）+ 辅助点光源（发光球）
    auto area_light_mat = make_shared<diffuse_light>(color(18, 18, 18));
    auto area_light = make_shared<quad>(point3(-1.4, 5.2, 0.6), vec3(2.8, 0, 0), vec3(0, 0, 2.8), area_light_mat);
    world.add(area_light);
    lights->add(area_light);

    auto fill_light_mat = make_shared<diffuse_light>(color(5, 5, 5));
    auto fill_light = make_shared<sphere>(point3(-4.5, 3.2, 2.5), 0.8, fill_light_mat);
    world.add(fill_light);
    lights->add(fill_light);

    // Disney 参数展示球
    auto disney_plastic = make_shared<disney_material>(color(0.86, 0.22, 0.20), 0.0, 0.35, 0.55, 0.0, 0.10);
    auto disney_metal = make_shared<disney_material>(color(0.96, 0.78, 0.24), 1.0, 0.22, 0.85, 0.0, 0.0);
    auto disney_clearcoat = make_shared<disney_material>(color(0.12, 0.42, 0.90), 0.0, 0.48, 0.55, 1.0, 0.20);

    world.add(make_shared<sphere>(point3(-3.6, 1.05, -0.3), 1.05, disney_plastic));
    world.add(make_shared<sphere>(point3(-0.8, 1.05, -0.3), 1.05, disney_metal));
    world.add(make_shared<sphere>(point3(2.0, 1.05, -0.3), 1.05, disney_clearcoat));

    // 纹理驱动 Disney 展示球（baseColor + metallic/roughness + normal map）
    auto tex_disney = make_shared<disney_material>(
        make_shared<image_texture>("solar system/earth_day_8k.jpg"),
        0.2, 0.65, 0.5, 0.25, 0.0
    );
    ...
    world.add(make_shared<sphere>(point3(4.8, 1.0, 0.2), 1.0, tex_disney));

    // 外部模型导入 + 贴图覆盖材质（关闭 embedded material，强制使用我们给的 Disney）
    ModelLoadOptions bunny_opt;
    bunny_opt.scale = vec3(2.2, 2.2, 2.2);
    bunny_opt.translate = vec3(-2.0, 0.0, -2.2);
    bunny_opt.center_model = true;
    bunny_opt.flip_winding = false;
    bunny_opt.import_embedded_materials = false;

    auto bunny_disney = make_shared<disney_material>(
        make_shared<image_texture>("solar system/jupiter_8k.jpg"),
        0.08, 0.42, 0.55, 0.20, 0.05
    );
    bunny_disney->set_metallic_roughness_texture(make_shared<image_texture>("solar system/mars_8k.jpg"));
    auto bunny = load_model_as_hittable("models/bunny.obj", bunny_disney, bunny_opt);
    world.add(bunny);

    ModelLoadOptions arm_opt;
    arm_opt.scale = vec3(1.6, 1.6, 1.6);
    arm_opt.translate = vec3(2.4, 0.0, -2.0);
    arm_opt.center_model = true;
    arm_opt.flip_winding = false;
    arm_opt.import_embedded_materials = false;

    auto arm_disney = make_shared<disney_material>(color(0.72, 0.74, 0.78), 0.0, 0.30, 0.55, 0.70, 0.10);
    auto armadillo = load_model_as_hittable("models/armadillo.obj", arm_disney, arm_opt);
    world.add(armadillo);
    ...
    cam.vfov = 30;
    cam.lookfrom = point3(0.3, 2.6, 9.2);
    cam.lookat = point3(0.5, 1.0, -1.0);
    cam.vup = vec3(0, 1, 0);
    ...
    cam.skybox_filename = "solar system/sky_8k.jpg";
    cam.sh_coeffs_filename = "galaxy_sh.txt";
    cam.use_spherical_harmonics = false;
    cam.enable_skybox = true;
