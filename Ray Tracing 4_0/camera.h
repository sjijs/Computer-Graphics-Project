...
class camera {
  public:
    ...
    bool enable_nee = true; // 是否启用下一事件估计
    bool enable_mis = true; // 是否启用多重重要性采样
    bool sample_world_for_direct_light = true; // 未指定light_sampler时是否用world作采样分布

    // 可选：显式设置用于直接光采样的几何集合（通常只放灯光物体）
    shared_ptr<hittable> light_sampler = nullptr;
  ...
  private:
                    ...
                    for (int j = start_j; j < end_j; ++j) {
                        ...
                        for (int i = start_i; i < end_i; ++i) {
                            ...
                            if (enable_progressive_rendering) {
                                ...
                                color averaged_color = accumulation_buffer[pixel_index] / total_samples;
                                encode_pixel_to_bgra(averaged_color, &display_buffer[static_cast<size_t>(pixel_index) * 4]);
                            } else {
                                // 传统模式:直接平均本帧样本
                                color scaled_color = pixel_color / current_spp;
                                accumulation_buffer[pixel_index] = scaled_color;
                                encode_pixel_to_bgra(scaled_color, &display_buffer[static_cast<size_t>(pixel_index) * 4]);
                            }
                        }
                    }
    ...
    static double power_heuristic(double a_pdf, double b_pdf) {
        auto a2 = a_pdf * a_pdf;
        auto b2 = b_pdf * b_pdf;
    ...
    color ray_color(const ray& r, int depth, const hittable& world) const {
        ...
        double prev_bsdf_pdf = 1.0;
        point3 prev_surface_p;

        const hittable* direct_light = nullptr;
        if (light_sampler) {
            direct_light = light_sampler.get();
        } else if (sample_world_for_direct_light) {
            direct_light = &world;
        }
        ...
        for (int bounce = 0; bounce < depth; ++bounce) {
            ...
            if (!world.hit(current_ray, interval(0.001, infinity), rec)) {
                L += throughput * sample_environment(current_ray.direction());
                break;
            }

            color Le = rec.mat->emitted(rec.u, rec.v, rec.p);
            if (Le.length_squared() > 0.0) {
                if (!enable_mis || prev_was_delta || !direct_light) {
                    L += throughput * Le;
                } else {
                    double light_pdf = direct_light->pdf_value(prev_surface_p, current_ray.direction());
                    double w = power_heuristic(prev_bsdf_pdf, light_pdf);
                    L += throughput * Le * w;
                }
            }

            bsdf_sample bs;
            if (!rec.mat->sample_bsdf(current_ray, rec, bs)) {
                break;
            }

            // 非delta材质执行直接光采样
            if (enable_nee && direct_light && !bs.is_delta) {
                vec3 light_dir_raw = direct_light->random(rec.p);
                vec3 light_dir = unit_vector(light_dir_raw);
                double light_pdf = direct_light->pdf_value(rec.p, light_dir);

                if (light_pdf > 1e-8) {
                    ray shadow_ray(rec.p, light_dir, current_ray.time());
                    hit_record light_hit;

                    if (world.hit(shadow_ray, interval(0.001, infinity), light_hit)) {
                        color light_Le = light_hit.mat->emitted(light_hit.u, light_hit.v, light_hit.p);
                        if (light_Le.length_squared() > 0.0) {
                            color f = rec.mat->eval_bsdf(current_ray, rec, light_dir);
                            double bsdf_pdf = rec.mat->pdf_bsdf(current_ray, rec, light_dir);
                            double cos_theta = std::max(0.0, dot(rec.shading_normal, light_dir));

                            if (f.length_squared() > 0.0 && cos_theta > 0.0) {
                                double w = enable_mis ? power_heuristic(light_pdf, bsdf_pdf) : 1.0;
                                L += throughput * f * light_Le * (cos_theta * w / light_pdf);
                            }
                        }
                    }
                }
            ...
            if (bs.is_delta) {
                throughput = throughput * bs.attenuation;
                current_ray = bs.scattered;
                prev_was_delta = true;
                prev_bsdf_pdf = 1.0;
                prev_surface_p = rec.p;
            } else {
                vec3 wi = unit_vector(bs.scattered.direction());
                double cos_theta = std::max(0.0, dot(rec.shading_normal, wi));
                if (bs.pdf <= 1e-8 || cos_theta <= 0.0)
                    break;

                throughput = throughput * bs.f * (cos_theta / bs.pdf);
                current_ray = bs.scattered;
            ...
            if (bounce > 3) {
                ...
                double q = std::clamp(1.0 - max_comp, 0.05, 0.95);
                if (random_double() < q)
                    break;
                throughput /= (1.0 - q);
            }
